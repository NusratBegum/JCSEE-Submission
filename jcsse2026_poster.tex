...
% Enhanced Adversarial Drift Detection for MLOps Feature Monitoring
...
\setbeamertemplate{headline}{
  ...
  \begin{beamercolorbox}[wd=\paperwidth, ht=12cm]{headline}
    \begin{center}
      \vskip1cm
      {\color{MahidolBlue}\VERYHuge\bfseries Enhanced Adversarial Drift Detection\\[0.3em]
...
\begin{itemize}
  ...
  \item \textbf{Gap:} No unsupervised detector simultaneously provides statistical rigor, feature-level attribution, and drift severity quantification.
\end{itemize}
\vskip0.5cm
{\color{MahidolBlue}\large\bfseries Research Question:}\\[6pt]
\textit{Can discriminative classification be extended with permutation testing and SHAP attribution to provide statistically rigorous, feature-attributed drift detection across diverse drift types?}
\end{block}
...
\begin{tikzpicture}[
  ...
  \node[box, below=of step1] (step2) {\textbf{Step 2: Discriminative Classifier}\\LightGBM distinguishes $W_{ref}$ vs $W_{cur}$\\AUC-ROC $>$ 0.7 $\Rightarrow$ suspect drift};
...
\large
EADD (\textbf{Enhanced Adversarial Drift Detection}) extends D3 with three contributions:
\begin{enumerate}
  \item \textbf{Adaptive Sequential Permutation Test (ASPT)} + prediction entropy monitoring ($H_{\text{pred}}$) for statistically rigorous confirmation
  \item \textbf{SHAP interaction-aware attribution} + \textbf{Drift Severity Index (DSI)} identifying which features caused drift and how severe
  \item \textbf{Intensive experimental validation} against 7 state-of-the-art detectors
\end{enumerate}
\end{exampleblock}
...
\midrule
Abrupt        & \color{AccentRed}\textbf{0/5} & \color{AccentGreen}\textbf{5/5} \\
Gradual       & \color{AccentRed}\textbf{0/5} & \color{AccentRed}\textbf{0/5} \\
Incremental   & \color{AccentRed}\textbf{0/5} & \color{AccentRed}\textbf{0/5} \\
Recurring     & \color{AccentRed}\textbf{0/5} & \color{AccentGreen}\textbf{5/5} \\
\midrule
\textbf{Overall} & \color{AccentRed}\textbf{0/20} & \color{AccentRed}\textbf{10/20} \\
\bottomrule
\end{tabular}
\end{table}

\vskip0.3cm
{\color{MahidolBlue}$\Rightarrow$} Multi-detector default run: EADD conservative (no triggers); D3 detects abrupt/recurring only.
\end{block}

\vskip1cm

% --- Experiment 2 ---
\begin{block}{4. Experiment 2 — 11 Real-World Datasets}
\large
...
\textbf{Dataset} & \textbf{EADD} & \textbf{D3} \\
 & \textbf{MTD} & \textbf{MTD} \\
\midrule
Insects Abrupt       & ---   & 152.5 \\
Insects Gradual      & --- & 9481 \\
Insects Incr.\ Abrupt   & ---    & 160   \\
Insects Incr.\ Balanced & ---   & --- \\
Insects Reoccurring  & ---  & 157   \\
SineClusters         & ---  & 229   \\
WaveformDrift2       & ---  & 169   \\
\midrule
\textbf{Known-drift summary} & \textbf{No triggers} & Active detections \\
\bottomrule
\end{tabular}
\end{table}

\vskip0.3cm
{\color{MahidolBlue}$\Rightarrow$} Real-world benchmark indicates EADD requires calibration for sensitivity.
\end{block}

\vskip1cm

% --- Experiment 3 ---
\begin{block}{5. Experiment 3 — SHAP Attribution Accuracy}
\large
...
\vskip0.3cm
{\color{MahidolBlue}$\Rightarrow$} \textbf{100\% feature attribution accuracy} across all three controlled drift patterns
\end{block}
...
\midrule
Temporal coverage     & \textbf{0/4}  & 2/4 \\
Real-world activity   & Conservative  & Active \\
Known-drift MTD       & --- & 152--9481 \\
...
\begin{itemize}
  \item EADD transforms drift detection from a \textbf{binary alarm} into an \textbf{actionable diagnostic tool} with feature-level attribution and drift severity quantification.
  \item \textbf{Strengths:} zero false alarms on stable streams, correct feature identification in all controlled scenarios, and statistically significant FA reduction vs D3 at $\tau=0.6$.
  \item \textbf{Limitations:} Conservative operating point in multi-detector benchmarks reduced detection sensitivity; computational overhead remains higher than threshold-based detectors.
...
\begin{enumerate}
  ...
  \item D.~Lukats et al., ``A benchmark and survey of fully unsupervised concept drift detectors on real-world data streams,'' \textit{Int. J. Data Sci. Analytics}, 2024.
